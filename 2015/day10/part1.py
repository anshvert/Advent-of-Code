...
For Second Part, apply process 50 times.
"""

initialString = "3113322113"
...
def generateSequence(string):
    ...
    for s in range(1,len(string)):
        if string[s] == string[s-1]:
            ...
            newString += (str(cf) + cs)
            cs,cf = string[s],1
    return newString + (str(cf) + cs)

for i in range(50):
    initialString = generateSequence(initialString)
print(len(initialString))