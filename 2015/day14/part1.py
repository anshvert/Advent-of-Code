...
def ReindeerOlympics(fileData):
    ...
    def getAmgSet(lftTime, speed, time):
        return speed * (min(time, lftTime))
    ...
    for stat in fileData:
        ...
        spanDist,spanTime = speed * time, time + rest
        distSets,amgTime = quesTime // spanTime, quesTime % spanTime
        amgSet = getAmgSet(amgTime,speed,time)
        speedDict[name] = distSets*spanDist + amgSet
