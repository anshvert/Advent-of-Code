...
def merge_intervals_special(intervalsA: List[Tuple[int, int, int]], intervalsB: List[Tuple[int, int, int]]) -> List[
    Tuple[int, int, int]]:
    """
...
if __name__ == '__main__':
    ...
    f_name = "inputs/day5.txt"
    node_intervals = []
    locations = []
    current_intervals_source_and_diff: List[Tuple[
        int, int, int]] = []  # will be like so -> [(startsource,endsource,difference)] difference = destination-source
                ...
                if "map" in line:  # then process all lines in current map
                    for line in file:
                        ...
                        if line == '' or "end" in line:
                            ...
                            #print(current_intervals_source_and_diff)
