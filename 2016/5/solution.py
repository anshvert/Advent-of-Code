...
door_id,ind = "ffykfhsq", 0
current_password = ['-1'] * 8

while '-1' in current_password:
    door_index = door_id + str(ind)
    door_index_hash: str = hashlib.md5(door_index.encode()).hexdigest()
    if door_index_hash[:5] == "00000" and door_index_hash[5].isdigit() and 0 <= int(door_index_hash[5]) <= 7:
        current_password[int(door_index_hash[5])] = door_index_hash[6] if current_password[int(door_index_hash[5])] == '-1' else current_password[int(door_index_hash[5])]
        print(current_password)
    ind += 1

print("Current Password for part 2:", "".join(current_password))